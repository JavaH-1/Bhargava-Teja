	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    try {
	        ...
	        PreparedStatement st = con.prepareStatement(
	            "INSERT INTO EmpTable(employeeName, employeeId, email, phone, department, dob, emergencyContact, doj, bank, country, state, username, password)"
	            + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
	        );
	    ...
	    try {
	        ...
	        if (!rs.isBeforeFirst()) {
	            ...
	            while (rs.next()) {
	                ...
	                json.append("\"country\": \"" + rs.getString("country") + "\"");
	                json.append("\"state\": \"" + rs.getString("state") + "\"");
	                json.append("},");
	            }
